 ...
 * route must return 501 LLM_NOT_ENABLED. Auth still applies: a missing token
 * yields 401 before the 501, and a non-admin hitting /admin/llm gets 403.
 *
...
describe("LLM routes (integration disabled by default)", () => {
  it("GET /api/v1/llm/me requires auth (401 before 501)", async () => {
    ctx = await setupTestApp();
    const res = await ctx.request().get("/api/v1/llm/me");
    expect(res.status).toBe(401);
  });

  it("GET /api/v1/llm/me returns 501 LLM_NOT_ENABLED when authenticated", async () => {
    ctx = await setupTestApp();
  ...
  it("GET /api/v1/admin/llm/bindings returns 501 for admin when disabled", async () => {
    ctx = await setupTestApp();
  ...
  it("POST /api/v1/llm/me/keys/:id/reveal is 501 when disabled", async () => {
    ctx = await setupTestApp();
  ...
  it("GET /api/v1/llm/models is 501 when disabled", async () => {
    ctx = await setupTestApp();
