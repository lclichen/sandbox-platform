...
All routes under `/api/v1`. Bearer token auth via `POST /auth/login`.

| Area | Endpoint | Who |
|------|----------|-----|
| Auth | `POST /auth/login` `/refresh` `/logout`, `GET /auth/me` | any |
| Containers | `POST/GET /containers`, `/:id/start\|stop`, `DELETE /:id` | owner |
...
| Admin misc | `GET /admin/dashboard`, `GET /admin/containers` | admin |
...
test/                    vitest E2E (34 tests, MockExecutor-backed)
```

## Testing

```bash
npm test    # 34 tests, fully offline, win32-compatible (Mock executor)
```
...
- No Web UI; the platform is API-only. The `pi-sandbox-extension` is the
  reference client.
